...
When an email triggers the flow, the mailhook node passes a payload with the following structure. Use it in later nodes (e.g. via `{{mailhookNodeId.output}}` or your trigger node’s output reference) to drive logic, extract data, or generate responses.
